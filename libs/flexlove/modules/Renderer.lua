...
function Renderer:draw(element, backdropCanvas)
  ...
  -- Read backgroundColor from the live element field rather than the cached
  -- Renderer copy, so direct assignments (`element.backgroundColor = X`) take
  -- effect without having to route through Element:setProperty.  Falls back
  -- to the cached value if the element somehow lacks the field.
  local drawBackgroundColor = element.backgroundColor or self.backgroundColor
  if element.animation then
    local anim = element.animation:interpolate()
    if anim.opacity then
      drawBackgroundColor =
        self._Color.new(drawBackgroundColor.r, drawBackgroundColor.g, drawBackgroundColor.b, anim.opacity)
    end
  end
...
function Renderer:drawText(element)
  ...
  if displayText and displayText ~= "" then
    ...
    if element.textWrap and (element.textWrap == "word" or element.textWrap == "char" or element.textWrap == true) then
      ...
      -- Regular print for non-wrapped text. textAlign controls the horizontal
      -- axis only (CSS text-align semantics); vertical positioning is done by
      -- the parent flex container.
      ty = contentY
      if element.textAlign == self._TextAlign.CENTER then
        tx = contentX + (textAreaWidth - textWidth) / 2
      elseif element.textAlign == self._TextAlign.END then
        tx = contentX + textAreaWidth - textWidth
      else
        tx = contentX
      end
