...
---@field gridTemplateColumns table? -- Array of track sizes for columns (number = px; string with "fr"/"px"/"%"). Overrides gridColumns.
---@field gridTemplateRows table? -- Array of track sizes for rows (number = px; string with "fr"/"px"/"%"). Overrides gridRows.
...
function Element.new(props)
  ...
  -- `display` controls whether the element participates in layout / paint /
  -- hit-testing.  Boolean prop, defaults to true.  Equivalent to CSS
  -- `display: none` when false: child is dropped from flex/grid calculations,
  -- skipped in draw and update, and excluded from getElementAtPosition.
  -- Use Element:setDisplay(v) to toggle at runtime (it invalidates layout
  -- automatically).  Direct mutation of `self.display` skips invalidation.
  self.display = props.display ~= false
  ...
  if self.positioning == Element._utils.enums.Positioning.GRID then
    ...
    -- Track-sizing arrays (CSS grid-template-{columns,rows}); when present,
    -- they override gridColumns/gridRows and drive per-track px/fr/% sizing.
    if type(props.gridTemplateColumns) == "table" and #props.gridTemplateColumns > 0 then
      self.gridTemplateColumns = props.gridTemplateColumns
      self.gridColumns = #props.gridTemplateColumns
    end
    if type(props.gridTemplateRows) == "table" and #props.gridTemplateRows > 0 then
      self.gridTemplateRows = props.gridTemplateRows
      self.gridRows = #props.gridTemplateRows
    end
    self.alignItems = props.alignItems or Element._utils.enums.AlignItems.STRETCH
...
--- `display = false` is equivalent to CSS `display: none`: the element is
--- skipped by layout / draw / hit-testing and its space is reclaimed.
--- `display = true` brings it back.  Always invalidates layout so the
--- parent reflows on the next frame.
---@param visible boolean
function Element:setDisplay(visible)
  local v = visible ~= false
  if self.display == v then return end
  self.display = v
...
function Element:draw(backdropCanvas)
  ...
  if self.opacity <= 0 or not self.display then
    return
  end
...
function Element:update(dt)
  -- Skip elements removed from layout (display = false).
