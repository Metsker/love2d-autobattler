...
function LayoutEngine:layoutChildren()
  ...
  for _, child in ipairs(self.element.children) do
    local isFlexChild = child.display
      and not (child.positioning == self._Positioning.ABSOLUTE and child._explicitlyAbsolute)
    if isFlexChild then
  ...
  if #flexChildren == 0 then
    ...
    for i, child in ipairs(self.element.children) do
      if child.display
        and child.positioning == self._Positioning.ABSOLUTE and child._explicitlyAbsolute then
